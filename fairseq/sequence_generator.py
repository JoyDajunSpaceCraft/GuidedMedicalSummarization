...
class SequenceGenerator(object):
    def __init__(
        ...
        self.bos = tgt_dict.bos() #if eos is None else eos
...
class EnsembleModel(torch.nn.Module):
    ...
    @torch.no_grad()
    def z_forward_encoder(self, encoder_input):
        if not self.has_encoder():
            return None
        return [model.z_forward_encoder(**encoder_input) for model in self.models]
    ...
    @torch.no_grad()
    def z_forward_decoder(self, tokens, encoder_outs, z_encoder_outs, temperature=1.):
        if len(self.models) == 1:
            return self.z_decode_one(
                tokens,
                self.models[0],
                encoder_outs[0] if self.has_encoder() else None,
                z_encoder_outs[0],
        ...
        log_probs = []
        avg_attn = None
        for model, encoder_out, z_encoder_out in zip(self.models, encoder_outs, z_encoder_outs):
            probs, attn = self.z_decode_one(
                tokens,
                model,
                encoder_out,
                z_encoder_out,
    ...
    def z_decode_one(
        self, tokens, model, encoder_out, z_encoder_out, incremental_states, log_probs,
        temperature=1.,
    ):
        if self.incremental_states is not None:
            decoder_out = list(model.z_forward_decoder(
                tokens, encoder_out=encoder_out, z_encoder_out=z_encoder_out, incremental_state=self.incremental_states[model],
            ))
        else:
            decoder_out = list(model.z_forward_decoder(tokens, encoder_out=encoder_out, z_encoder_out=z_encoder_out))
        decoder_out[0] = decoder_out[0][:, -1:, :]
        if temperature != 1.:
            decoder_out[0].div_(temperature)
        attn = decoder_out[1]
...
class GuidedSequenceGenerator(object):
    def __init__(
        self,
        tgt_dict,
        beam_size=1,
    ...
    def _generate(
        ...
        **kwargs
        ...
        src_tokens = encoder_input['src_tokens']
        src_lengths = (src_tokens.ne(self.eos) & src_tokens.ne(self.pad)).long().sum(dim=1)
        z_tokens = encoder_input['z_tokens']
        z_lengths = (z_tokens.ne(self.eos) & z_tokens.ne(self.pad)).long().sum(dim=1)
        input_size = src_tokens.size()
        # batch dimension goes first followed by source lengths
        bsz = input_size[0]
        src_len = input_size[1]
        ...
        # compute the encoder output for each beam
        #encoder_outs, z_encoder_outs = model.z_forward_encoder(encoder_input)
        a = model.z_forward_encoder(encoder_input)
        encoder_outs = [a[0][0]]
        z_encoder_outs = [a[0][1]]
        new_order = torch.arange(bsz).view(-1, 1).repeat(1, beam_size).view(-1)
        new_order = new_order.to(src_tokens.device).long()
        encoder_outs = model.reorder_encoder_out(encoder_outs, new_order)
        z_encoder_outs = model.reorder_encoder_out(z_encoder_outs, new_order)
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                model.reorder_incremental_state(reorder_state)
                encoder_outs = model.reorder_encoder_out(encoder_outs, reorder_state)
                z_encoder_outs = model.reorder_encoder_out(z_encoder_outs, reorder_state)

            lprobs, avg_attn_scores = model.z_forward_decoder(
