...
class GuidedBARTHubInterface(nn.Module):
    ...
    def _build_sample(self, src_tokens: List[torch.LongTensor], zs: List[torch.LongTensor]):
    ...
    def sample(self, sentences: List[str], zs: List[str], beam: int = 1, verbose: bool = False, **kwargs) -> str:
        print(sentences)
    ...
    def generate(self, tokens: List[torch.LongTensor], zs: List[torch.LongTensor], beam: int = 5, verbose: bool = False, **kwargs) -> torch.LongTensor:
        sample = self._build_sample(tokens, zs)
